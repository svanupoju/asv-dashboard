...
@Component({
  ...
  styleUrls: ['./car-tracking.component.scss']
})
export class CarTrackingComponent implements OnDestroy {
  private readonly vehicleType = 'car';
  private readonly serviceIntervalKm = 10000;
  private readonly serviceIntervalMonths = 12;
  ...
  get selectedVehicleName(): string {
    return this.selectedVehicle?.name ?? 'Car';
  }

  get filteredEntries(): CarFuelEntry[] {
  ...
  private get selectedVehicleEntries(): CarFuelEntry[] {
    return this.entries.filter((entry) => this.entryBelongsToSelectedVehicle(entry.vehicleId));
  }

  private get positiveAmountEntries(): CarFuelEntry[] {
    return this.filteredEntries.filter((entry) => entry.amount > 0);
  }

  private get latestPositiveEntry(): CarFuelEntry | null {
  ...
  async updateSelectedVehicle(): Promise<void> {
    ...
    if (exists) {
      this.trackerError = 'A car with that name already exists.';
      return;
    }
    ...
    try {
      ...
      this.trackerError = error?.message ? String(error.message) : 'Failed to update car.';
    } finally {
  ...
  async deleteSelectedVehicle(): Promise<void> {
    ...
    const confirmed = window.confirm(
      `Delete ${vehicleName || 'this car'} and all its fuel entries, service records, documents, and uploaded files?`
    );
    if (!confirmed) {
      return;
    }
    ...
    try {
      ...
      if (driveDeletionErrors.length) {
        this.trackerError = `Car deleted from app, but some Google Drive files could not be removed: ${driveDeletionErrors.join(' ')}`;
      }
    } catch (error: any) {
      console.error('Failed to delete car profile:', error);
      this.trackerError = error?.message ? String(error.message) : 'Failed to delete car.';
    } finally {
  ...
  async addVehicle(): Promise<void> {
    ...
    try {
      ...
      console.error('Failed to add car profile:', error);
      this.trackerError = error?.message ? String(error.message) : 'Failed to add car.';
    } finally {
  ...
  async addEntry(): Promise<void> {
    ...
    try {
      await addDoc(collection(this.firestore, 'monthlyDebits'), {
        ...
        notes: `Car Tracking - ${this.selectedVehicleName}`,
        email: this.userEmail,
        vehicleType: 'car',
      ...
      console.error('Failed to save car entry:', error);
      this.trackerError = error?.message ? String(error.message) : 'Failed to save car entry.';
    } finally {
  ...
  async saveServiceSchedule(): Promise<void> {
    ...
    try {
      ...
      console.error('Failed to save car service schedule:', error);
      this.trackerError = error?.message ? String(error.message) : 'Failed to save car service schedule.';
    } finally {
  ...
  async saveVehicleDocument(documentType: VehicleDocumentType): Promise<void> {
    ...
    try {
      ...
      console.error('Failed to save car document:', error);
      this.trackerError = error?.message ? String(error.message) : 'Failed to save car document.';
    } finally {
  ...
  async onDocumentFilesSelected(event: Event): Promise<void> {
    ...
    try {
      ...
      console.error('Failed to upload car document:', error);
      this.trackerError = error?.message ? String(error.message) : 'Failed to upload car document.';
    } finally {
  ...
  async deleteDocumentUpload(upload: VehicleDocumentUpload): Promise<void> {
    ...
    try {
      ...
      console.error('Failed to delete car document upload:', error);
      this.trackerError = error?.message ? String(error.message) : 'Failed to delete car document.';
    } finally {
  ...
  async deleteEntry(entry: CarFuelEntry): Promise<void> {
    ...
    try {
      ...
      console.error('Failed to delete car entry:', error);
      this.trackerError = error?.message ? String(error.message) : 'Failed to delete car entry.';
    } finally {
  ...
  private subscribeToVehicles(): void {
    ...
    this.vehiclesUnsubscribe = onSnapshot(vehiclesRef, (snapshot) => {
      ...
      console.error('Failed to load car profiles:', error);
      this.trackerError = error?.message ? String(error.message) : 'Failed to load cars.';
      this.vehicles = [];
      this.selectedVehicleId = '';
      this.selectedVehicleNameDraft = '';
      this.resetSelectedVehicleState();
  ...
  private subscribeToEntries(): void {
    ...
    this.entriesUnsubscribe = onSnapshot(rowsRef, (snapshot) => {
      ...
      console.error('Failed to load car entries:', error);
      this.trackerError = error?.message ? String(error.message) : 'Failed to load car entries.';
      this.entries = [];
    });
  }
  ...
  private subscribeToServiceSchedule(): void {
    ...
    this.serviceScheduleUnsubscribe = onSnapshot(scheduleRef, (snapshot) => {
      ...
      console.error('Failed to load car service schedule:', error);
      this.trackerError = error?.message ? String(error.message) : 'Failed to load car service schedule.';
    });
  }
  ...
  private subscribeToDocuments(): void {
    ...
    this.documentsUnsubscribe = onSnapshot(documentsRef, (snapshot) => {
      ...
      this.trackerError = error?.message ? String(error.message) : 'Failed to load car documents.';
    });
  }
  ...
  private subscribeToDocumentUploads(): void {
    ...
    this.uploadsUnsubscribe = onSnapshot(uploadsRef, (snapshot) => {
      ...
      console.error('Failed to load car document uploads:', error);
      this.trackerError = error?.message ? String(error.message) : 'Failed to load car document uploads.';
      this.documentUploads = [];
    });
  }
  ...
  private async ensureDefaultVehicle(): Promise<void> {
    ...
    try {
      ...
      console.error('Failed to create default car profile:', error);
    } finally {
  ...
  private async setPreferredVehicle(vehicleId: string): Promise<void> {
    ...
    try {
      ...
      console.error('Failed to persist preferred car selection:', error);
    }
  }
